...
def add_officers(request):
    ...
    # customer_name = Customers.objects.all().distinct('customer_name')
    print('11111')
    print(customer)
...
# @cache_control(no_cache=True, must_revalidate=True, no_store=True)
def manage_transfer_page(request):
...
def manage_transfer_action(request):
    if request.method != "POST":
        HttpResponse("Invalid Method")
    else:
        personnel_id = request.POST.get('pno')
        print('11111')
        print(personnel_id)
        transfer = Transfers.objects.all()
        customer = Customers.objects.all()
        personnelmaster = PersonnelMasters.objects.get(personnel_id=personnel_id)
        unit = PersonnelMasters.objects.filter(personnel_id=personnel_id).values_list('unit')
        context = {
            "transfers" : transfer,
            "personnelmasters":personnelmaster,
            "customers" : customer,
...
# Code pending for insert into transfer_trails
# Django handles for updates and inserts
# Make sure explicit insert happens in the trail table
def manage_transfer_save(request):
    if request.method != "POST":
        ...
        personnelmaster = PersonnelMasters.objects.get(personnel_id=personnel_id)
        # pno = PersonnelMasters.objects.get(personnel_id=personnel_id).values
        # if transfer_var:
        ...
        # try:
            ...
            # transfer = Transfers.objects.get(personnel_id=personnel_id)
            ...
            # transfer.porposed_appointment = proposed_appointment
            # transfer.dtbr = dtbr
            # transfer.status = 1
            # transfer.date_initiated = datetime.now()
        transfer_object = Transfers(
            pno = personnelmaster.pno,
            personnel_id = personnel_id,
            name = personnelmaster.name,
            rank = personnelmaster.rank,
            present_unit = personnelmaster.unit,
            present_appointment = personnelmaster.appointment,
            pu_outgoing_yn = "N",
        ...
        messages.success(request, "Course Updated Successfully.")
        return redirect('manage_transfer_page')
...
# Include a p.no search button in here.
# The user can not select from a drop down of 20000 persons.
# a common pno search function

def officers_marked_for_transfer(request):
    transfer = Transfers.objects.filter(status = 1)
    personnelmaster = PersonnelMasters.objects.all()
    context = { 
            "transfer" : transfer,
            "personnelmasters" : personnelmaster,
    }
    return render(request,"hod_template/officers_marked_for_transfer.html",context)

def edit_transfer(request, transfers_id):
    transfer = Transfers.objects.get(transfers_id=transfers_id)
    context = {
        "transfers": transfer,
        "transfers_id": transfers_id
    }
    return render(request, 'hod_template/edit_transfer_template.html', context)
